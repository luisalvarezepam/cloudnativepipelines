...
# Create Linux application service
...
resource "azurerm_linux_web_app_slot" "webapp" {
  ...
  site_config {
    ...
    dynamic "application_stack" {
      ...
      content {
        docker_image        = lookup(var.web_app_slots[count.index].application_stack, "docker_image", null)
        docker_image_tag    = lookup(var.web_app_slots[count.index].application_stack, "docker_image_tag", null)
        dotnet_version      = lookup(var.web_app_slots[count.index].application_stack, "dotnet_version", null)
        java_server         = lookup(var.web_app_slots[count.index].application_stack, "java_server", null)
        java_server_version = lookup(var.web_app_slots[count.index].application_stack, "java_server_version", null)
        ...
        ruby_version        = lookup(var.web_app_slots[count.index].application_stack, "ruby_version", null)
